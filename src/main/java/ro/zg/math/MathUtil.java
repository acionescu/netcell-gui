...
//	    
//	    List<Point2D> p1 = new ArrayList<Point2D>(path);
//	    List<Point2D> p2 = new ArrayList<Point2D>(path);
//	    
//	    newSources.add(yAligned);
//	    
//	    double dist1 = getShortestPath2(oldSource, newSources, obstacles, minDistToObstacle, p1);
//	    double dist2 = getShortestPath2(newSources,destList, obstacles, minDistToObstacle, p2);
//	    path.clear();
//	    path.addAll(p1);
//	    
//	}
...
//		}
    ...
    private static List<Point2D> getPathOptions(Point2D source,Collection<Rectangle2D> obstacles,Rectangle2D obstacle,double minDistToObstacle,List<Point2D> path){
	double sx = source.getX();
	double sy = source.getY();
	List<Point2D> newSources = new ArrayList<Point2D>();
	Point2D newPoint = null;
	/* right */
	newPoint = new Point2D.Double(obstacle.getMaxX() + minDistToObstacle, sy);
	if (!path.contains(newPoint) && ( obstacle.contains(source) || !checkOverlapping(obstacles, new Line2D.Double(source, newPoint)) )) {
	    newSources.add(newPoint);
	}
